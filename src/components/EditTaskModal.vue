  ...
  <transition name="modal-animation">
    <div v-show="modalEditActive" class="modal">
      <transition name="modal-animation-inner">
        <div v-show="modalEditActive" class="modal-inner" ref="modal">
          <div class="modal-head">
            <h2 class="modal-title">Edit Existing Task</h2>
            <font-awesome-icon
              @click="$emit('close')"
              :icon="['fas', 'x']"
...
<script>
export default {
  props: ["modalEditActive", "id"],
  watch: {
    modalEditActive(isActive) {
      if (isActive && this.id) {
        this.fetchData();
      }
    },
  ...
  methods: {
    async fetchData() {
      ...
      this.priority = this.task.data.Priority;
      this.deadline = this.task.data.Deadline;
    },
